# DAW — Ejercicios

Ejercicios prácticos siguiendo el stack real de Desarrollo de Aplicaciones Web,
orientados al mundo laboral: HTML → CSS → JavaScript → API REST → SQL → Node/Express → React.

Cada ejercicio vive en su propia carpeta, con todo lo que necesita (HTML, CSS, JS) junto.

## Progreso

### Bloque 1 — Frontend: HTML y CSS
- [x] 1.1 — Formulario de contacto semántico (HTML + validación HTML5)
- [x] 1.2 — Selectores CSS (etiqueta, clase, id, atributo, descendiente, pseudoclase)
- [x] 1.3 — Layout con Flexbox
- [ ] 1.4 — Bootstrap o Tailwind

### Bloque 2 — JavaScript
- [x] 2.1 — Validación de formulario (eventos, funciones reutilizables)
- [x] 2.2 — LocalStorage (persistencia de datos)
- [x] 2.3 — Envío con Fetch (conectar con una API)
- [x] 2.4 — Variables, tipos y operadores
- [ ] 2.5 — Arrays y objetos
- [ ] 2.6 — Control de flujo
- [ ] 2.7 — JSON

### Bloque 3 — API REST
- [ ] 3.1 — Conceptos (verbos HTTP, status codes)
- [ ] 3.2 — Consumo de API con fetch/async-await

### Bloque 4 — Backend y base de datos
- [ ] 4.1 — SQL y MySQL
- [ ] 4.2 — Node.js
- [ ] 4.3 — Express
- [ ] 4.4 — Conexión Express + MySQL
- [ ] 4.5 — EJS

### Bloque 5 — Framework moderno
- [ ] 5.1 — React

## Estructura del repo

```
DAW/
├── 01-frontend/
├── 02-javascript/
├── 03-api-rest/
├── 04-backend-node-express/
└── 05-react/
```
